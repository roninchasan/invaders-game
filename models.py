...
class Ship(GImage):
    ...
    def __init__(self):
        """
        Initializes a single alien object.
    ...
    def collides(self,bolt):
        """
        Returns: True if the bolt was fired by an and collides with the player

        Parameter bolt: The laser bolt to check
        Precondition: bolt is of class Bolt
        """

        if ((self.contains((bolt.x,bolt.y)) or self.contains(((bolt.x + BOLT_WIDTH),
        bolt.y)) or self.contains((bolt.x,(bolt.y + BOLT_HEIGHT))) or
        self.contains(((bolt.x + BOLT_WIDTH),(bolt.y + BOLT_HEIGHT))))
        and bolt._isPlayerBolt==False):
            return True
        else:
            return False
...
class Bolt(GRectangle):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
        _isPlayerBolt: Whether the bolt object is fired by the player or not [bool]
    """
    ...
    def isPlayers(self):
        """
        Returns True if bolt object was launched by the player. Returns false otherwise.
        """
        x = self._isPlayerBolt
        return x

    # INITIALIZER TO SET THE VELOCITY
    def __init__(self,x,y, isPlayer):
        """
        Initializes a single bolt object.

        This method should make sure that all of the attributes satisfy the given
        invariants. It sets the x, y, height, width, and color (fillcolor and linecolor) of each bolt.
        """
        # IMPLEMENT ME
        super().__init__(x=x,y=y,width=BOLT_WIDTH,height=BOLT_HEIGHT, fillcolor='red', linecolor='red')
        self._isPlayerBolt = isPlayer
        self._velocity = self.velo()

    # ADD MORE METHODS (PROPERLY SPECIFIED) AS NECESSARY
    def velo(self):
        """
        Determines if self._velocity is a player-launched bolt or not, and assigns BOLT_SPEED accordingly.
